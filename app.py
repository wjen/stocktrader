...
def index():
    ...
    # query all stocks from user
    rows_list = db.execute(
        "SELECT symbol, shares FROM portfolios WHERE id=:id", id=session['user_id'])
...
def buy():
    ...
    if request.method == "GET":
        ...
        if total_cost < cash_available:
            ...
            rows = db.execute("SELECT * FROM portfolios WHERE id=:id AND symbol=:symbol",
...
def quote():
    if request.method == 'GET':
        ...
        # error handling for quote route
...
def sell():
    ...
    if request.method == "GET":
        ...
        shares_already_list = db.execute(
            'SELECT shares FROM portfolios WHERE id=:id AND symbol=:symbol', id=session['user_id'], symbol=symbol)
        ...
        db.execute('UPDATE users SET cash=cash+:increase WHERE id=:id',
                   increase=increase_cash, id=session['user_id'])
...
def changepw():
    if request.method == "GET":
        ...
        db.execute("UPDATE users SET hash=:hash WHERE id=:id",
                   hash=generate_password_hash(new_pw), id=session['user_id'])
...
def deposit():
    ...
    if request.method == "GET":
        ...
        # query the amount of cash for user
        cash_list = db.execute(
            "SELECT cash FROM users WHERE id=:id", id=session['user_id'])
        add_funds = int(request.form.get('add_funds'))
        if not add_funds or add_funds <= 0:
            return apology('Invalid Amount')
        db.execute('UPDATE users SET cash= cash + :add_funds WHERE id=:id',
                   add_funds=add_funds, id=session['user_id'])
